-- fixes onboarding profile creation and feed category casts.
...
alter table public.users_public drop constraint if exists users_public_username_key;
alter table public.users_public drop constraint if exists unique_username;

drop index if exists public.unique_username;
drop index if exists public.users_public_username_key;

create unique index if not exists users_public_username_unique_ci
  on public.users_public (lower(username))
  where username is not null;

drop policy if exists "user can insert own public profile" on public.users_public;
...
-- part b: fixed get_personalized_feed (enum/text compatibility)
