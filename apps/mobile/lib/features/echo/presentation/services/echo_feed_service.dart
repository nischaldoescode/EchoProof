...
class EchoFeedService extends ChangeNotifier {
  ...
  Future<void> loadFeed() async {
    ...
    try {
      ...
      AppLogger.info('feed: loaded ${results.length} echoes');
    } catch (e) {
      _loadState = FeedLoadState.error;
      _error     = 'could not load feed';
      AppLogger.error('feed: load failed', e);
    }
    notifyListeners();
  }
  ...
  void applyOptimisticInteraction({
    ...
    _echoes = _echoes.map((echo) {
      ...
      return echo.copyWith(
        supportCount:   type == 'support'   ? echo.supportCount + 1   : echo.supportCount,
        challengeCount: type == 'challenge' ? echo.challengeCount + 1 : echo.challengeCount,
      );
    }).toList();
    notifyListeners();
  }
